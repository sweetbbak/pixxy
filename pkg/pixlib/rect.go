...
func R(minX, minY, maxX, maxY float64) Rect {
	return NewRect(Vec2{minX, minY}, Vec2{maxX, maxY})
}
...
func (r Rect) CenterOrigin(v Vec2, z float64) Vec3 {
...
// Size returns the vector2 of width and height of the Rect.
...
func (r Rect) Moved(delta Vec2) Rect {
	return Rect{
		Min: r.Min.Add(delta),
		Max: r.Max.Add(delta),
	}
...
func (r Rect) Resized(anchor, size Vec2) Rect {
	if r.W()*r.H() == 0 {
		panic(fmt.Errorf("(%T).Resize: zero area", r))
	}
	fraction := Vec2{size.X / r.W(), size.Y / r.H()}
	return Rect{
...
func (r Rect) ResizedMin(size Vec2) Rect {
	return Rect{
		Min: r.Min,
		Max: r.Min.Add(size),
	}
...
func (r Rect) Contains(u Vec2) bool {
...
func (r Rect) EachVec(dir Vec2, fn func(p Vec2)) {
	if dir.X == 0 {
		dir.X = 1
	}
